...
public class HatGenerator {
    ...
    private final Map<String, Float> mHatProbabilities;
    ...
    private void seedHats() {
        ...
        for (int k = 0; k < HAT_INDICES.length; k++) {
            ...
            float hatProbability = mServices.constantManager().getFloat(
                    key(s, i) + "_hat_probability");
            mHatProbabilities.put(key(s, i), mServices.constantManager()
                    .getFloat(key(s, i) + "_hat_probability"));
            if (hatProbability < smallestProbability) {
                smallestProbability = hatProbability;
            }
        }

        mHatsPerProbability = mServices.constantManager().getFloat(
                "min_hats_per_type")
                / smallestProbability;
    ...
    private void generateNewHats() {
        for (String s : mHatPool.keySet()) {
            ...
            int hatsToGenerate = (int) Math.ceil(mHatsPerProbability
                    * mServices.constantManager().getFloat(
                            s + "_hat_probability"));
            for (int i = 0; i < hatsToGenerate; i++) {
                hats.add(new Hat(hatSprite(index(s)), tint(color(s)), s,
                        mServices));
            }
        }
    }
    ...
    private Sprite hatSprite(int spriteIndex) {
        Sprite s = mServices.contentManager().loadPackedSprite(
                "hat" + spriteIndex);
